...
int matrix(WINDOW *mainwin) {
	...
	mx_size(mainwin, &cols_size, &rows_size);
	start_color();
	...
	wclear(mainwin);
	mx_printmass(mainwin, cols_size, rows_size);
	for (int i = 0; i < rows_size; i++) {
		for (int j = 0; j < cols_size; j++) {
			matrix[i].arr[j] = mx_rand_num();
		}
		...
		for (int i = 0; i < rows_size; i++) {
			for (int j = 0; j < cols_size; j++) {
				if (j >= matrix[i].start && j <= matrix[i].finish ) {
					...
					if (j == matrix[i].start) matrix[i].arr[j] = mx_rand_num();
					if (j == matrix[i].finish) matrix[i].arr[j] = mx_rand_num();
				}
			}
			matrix[i].start++;
			matrix[i].finish++;
